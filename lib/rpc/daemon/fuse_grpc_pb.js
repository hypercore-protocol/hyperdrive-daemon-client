...
function serialize_KeyRequest(arg) {
  if (!(arg instanceof daemon_fuse_pb.KeyRequest)) {
    throw new Error('Expected argument of type KeyRequest');
  }
  return Buffer.from(arg.serializeBinary());
}

function deserialize_KeyRequest(buffer_arg) {
  return daemon_fuse_pb.KeyRequest.deserializeBinary(new Uint8Array(buffer_arg));
}

function serialize_KeyResponse(arg) {
  if (!(arg instanceof daemon_fuse_pb.KeyResponse)) {
    throw new Error('Expected argument of type KeyResponse');
  }
  return Buffer.from(arg.serializeBinary());
}

function deserialize_KeyResponse(buffer_arg) {
  return daemon_fuse_pb.KeyResponse.deserializeBinary(new Uint8Array(buffer_arg));
}
...
var FuseService = exports.FuseService = {
  ...
  key: {
    ...
    requestStream: false,
    responseStream: false,
    requestType: daemon_fuse_pb.KeyRequest,
    responseType: daemon_fuse_pb.KeyResponse,
    requestSerialize: serialize_KeyRequest,
    requestDeserialize: deserialize_KeyRequest,
    responseSerialize: serialize_KeyResponse,
    responseDeserialize: deserialize_KeyResponse,
  },
};
