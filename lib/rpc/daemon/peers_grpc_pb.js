...
function serialize_ListPeersRequest(arg) {
  if (!(arg instanceof daemon_peers_pb.ListPeersRequest)) {
    throw new Error('Expected argument of type ListPeersRequest');
  }
  return Buffer.from(arg.serializeBinary());
}

function deserialize_ListPeersRequest(buffer_arg) {
  return daemon_peers_pb.ListPeersRequest.deserializeBinary(new Uint8Array(buffer_arg));
}

function serialize_ListPeersResponse(arg) {
  if (!(arg instanceof daemon_peers_pb.ListPeersResponse)) {
    throw new Error('Expected argument of type ListPeersResponse');
  }
  return Buffer.from(arg.serializeBinary());
}

function deserialize_ListPeersResponse(buffer_arg) {
  return daemon_peers_pb.ListPeersResponse.deserializeBinary(new Uint8Array(buffer_arg));
}
...
var PeersService = exports.PeersService = {
  ...
  listPeers: {
    path: '/Peers/listPeers',
    requestStream: false,
    responseStream: false,
    requestType: daemon_peers_pb.ListPeersRequest,
    responseType: daemon_peers_pb.ListPeersResponse,
    requestSerialize: serialize_ListPeersRequest,
    requestDeserialize: deserialize_ListPeersRequest,
    responseSerialize: serialize_ListPeersResponse,
    responseDeserialize: deserialize_ListPeersResponse,
  },
  watchPeers: {
