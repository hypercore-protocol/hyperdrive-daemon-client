...
function fromDriveStats (driveStats) {
  return driveStats.getStats().map(st => getMountStats(st))
...
function toDriveStats (stats) {
  ...
  function setCoreStats(coreStats, stats) {
    driveStats.setKey(stats.key)
    driveStats.setPeers(stats.peers)
    driveStats.setUploadedbytes(stats.uploadedBytes)
    driveStats.setDownloadedbytes(stats.downloadedBytes)
    driveStats.setUploadedbytescumulative(stats.uploadedBytesCumulative)
    driveStats.setDownloadedbytescumulative(stats.downloadedBytesCumulative)
  }
}
