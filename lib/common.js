...
function fromDiffEntry (diffEntry) {
  ...
  if (mnt) diff.mount = mnt
  if (stat) diff.stat = stat
...
function toDownloadProgress (stats) {
  const p = new rpc.drive.messages.DownloadProgress()
  p.setCompleted(!!(stats.completed))
  p.setCancelled(!(stats.completed))
...
function fromDownloadProgress (rsp) {
  return {
    completed: rsp.getCompleted(),
    cancelled: rsp.getCancelled(),
