...
module.exports = class DriveClient {
  ...
  _addCompatMethods (drive) {
    ...
    // TODO: This needs to be fixed to not use _putStat (but it needs to be atomic).
    drive.deleteMetadata = (path, keys, cb) => {
      return maybe(cb, new Promise((resolve, reject) => {
        drive.stat(path, (err, st) => {
          if (err) return reject(err)
          const metadata = st.metadata ? { ...st.metadata } : {}
          for (const key of keys) {
            delete metadata[key]
          }
          st.metadata = metadata
          return drive.drive._putStat(path, st, err => {
            if (err) return reject(err)
            return resolve(null)
          })
        })
